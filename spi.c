...
int spi_open(spi_t *spi, const char *path, unsigned int mode, uint32_t max_speed) {
    return spi_open_advanced(spi, path, mode, max_speed, MSB_FIRST, 16, 0);
}
...
int spi_transfer(spi_t *spi, const void *txbuf, void *rxbuf, size_t len) {
    ...
    spi_xfer.len = len;
    spi_xfer.delay_usecs = 10;
    spi_xfer.speed_hz = 500000;
